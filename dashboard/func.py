...
class DataAnalyzer:
    ...
    def create_rfm_df(self):
        # Calculate the snapshot date (the day after the most recent order)
        snapshot_date = self.df['order_purchase_timestamp'].max() + pd.Timedelta(days=1)

        # Group by customer_id and calculate the RFM metrics
        rfm_df = self.df.groupby('customer_id').agg({
            # Recency: difference between snapshot date and the most recent order date
            'order_purchase_timestamp': lambda x: (snapshot_date - x.max()).days,
            # Frequency: count of orders per customer
            'order_id': 'count',
            # Monetary: sum of payment values per customer
            'payment_value': 'sum'
        }).reset_index()
